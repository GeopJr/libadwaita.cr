...
module Gtk
  ...
  class ColorButton < Widget
    ...
    def self.new_with_rgba(rgba : Gdk::RGBA) : Gtk::Widget
    ...
    struct ActivateSignal
      ...
      def connect(block : Proc(Gtk::ColorButton, Nil))
        box = ::Box.box(block)
        slot = ->(lib_sender : Pointer(Void), box : Pointer(Void)) {
          sender = Gtk::ColorButton.new(lib_sender, GICrystal::Transfer::None)
          ::Box(Proc(Gtk::ColorButton, Nil)).unbox(box).call(sender)
        }
      ...
      def connect_after(block : Proc(Gtk::ColorButton, Nil))
        box = ::Box.box(block)
        slot = ->(lib_sender : Pointer(Void), box : Pointer(Void)) {
          sender = Gtk::ColorButton.new(lib_sender, GICrystal::Transfer::None)
          ::Box(Proc(Gtk::ColorButton, Nil)).unbox(box).call(sender)
