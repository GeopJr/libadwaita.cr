...
module Gtk
  ...
  class ShortcutsWindow < Window
    ...
    GICrystal.define_new_method(ShortcutsWindow, g_object_get_qdata, g_object_set_qdata)
    ...
    struct CloseSignal
      ...
      def connect(handler : Proc(Gtk::ShortcutsWindow, Nil))
        _box = ::Box.box(handler)
        handler = ->(_lib_sender : Pointer(Void), _lib_box : Pointer(Void)) {
      ...
      def connect_after(handler : Proc(Gtk::ShortcutsWindow, Nil))
        _box = ::Box.box(handler)
        handler = ->(_lib_sender : Pointer(Void), _lib_box : Pointer(Void)) {
          _sender = Gtk::ShortcutsWindow.new(_lib_sender, GICrystal::Transfer::None)
          ::Box(Proc(Gtk::ShortcutsWindow, Nil)).unbox(_lib_box).call(_sender)
