module GObject
  ...
  class TypePluginClass
    ...
    def base_iface : GObject::TypeInterface
      # Property getter
      _var = (@pointer + 0).as(Pointer(LibGObject::TypeInterface))
      GObject::TypeInterface.new(_var.value, GICrystal::Transfer::Full)
    end

    def base_iface=(value : GObject::TypeInterface)
      # Property setter
      _var = (@pointer + 0).as(Pointer(LibGObject::TypeInterface)).value = value.to_unsafe
      value
    end
