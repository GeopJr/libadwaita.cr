...
module Gdk
  ...
  def raise_exception(error : Pointer(LibGLib::Error))
    ...
    if error_domain == LibGLib.g_quark_try_string("gdk-gl-error-quark")
      raise GLError::NotAvailable.new(error) if error_code == 0
      raise GLError::UnsupportedFormat.new(error) if error_code == 1
    ...
    if error_domain == LibGLib.g_quark_try_string("gdk-vulkan-error-quark")
      raise VulkanError::Unsupported.new(error) if error_code == 0
      raise VulkanError::NotAvailable.new(error) if error_code == 1
    end

    Cairo.raise_exception(error)
    PangoCairo.raise_exception(error)
    Pango.raise_exception(error)
    Gio.raise_exception(error)
    GdkPixbuf.raise_exception(error)
  end
