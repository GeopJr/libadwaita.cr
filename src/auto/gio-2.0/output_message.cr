module Gio
  ...
  class OutputMessage
    ...
    def vectors : Gio::OutputVector
      # Property getter
      _var = (@pointer + 8).as(Pointer(Pointer(LibGio::OutputVector)))
      Gio::OutputVector.new(_var.value, GICrystal::Transfer::Full)
    end

    def vectors=(value : Gio::OutputVector)
      # Property setter
      _var = (@pointer + 8).as(Pointer(Pointer(LibGio::OutputVector))).value = value.to_unsafe
      value
    end
